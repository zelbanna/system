...
class Munin(object):
 ...
 def discover(self, astart, astop, adomain, ahandler = '127.0.0.1'):
  sysLogMsg("MuninDiscover: " + astart + " -> " + astop + ", for domain '" + adomain + "', handler:" + ahandler)
  hostsmods = '/var/tmp/munin.hosts.conf'
  muninadds = '/var/tmp/munin.addplugins'

  muninconfdict = self.getConf()
   ...
   with open(muninadds, 'a') as muninadd:
    if found[3] == "VMware":
     if not fqdn in muninconfdict:
      self.appendConf(fqdn, ahandler, "no") 
